...
import { brandAPI, categoryAPI, modelAPI, storeAPI, bikeAPI } from "../api/apiConfig";
...
const AddBikeForm = () => {
  ...
  const handleChange = (e) => {
    ...
    if (type === "checkbox") {
      ...
      if (name === "vehicleImages") {
        setBikeData({ ...bikeData, [name]: Array.from(files) });
      } else {
        setBikeData({ ...bikeData, [name]: files[0] });
      }
    } else {
  ...
  // ✅ Submit bike data to backend
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      console.log("🚀 Submitting Bike Data:", bikeData);
      const response = await bikeAPI.create(bikeData);
      console.log("✅ Bike added:", response.data);
      alert("✅ Bike Added Successfully!");
    } catch (error) {
      console.error("❌ Error adding bike:", error);
      alert("Error adding bike. Check console for details.");
    }
  };
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-6">
      <div className="bg-white rounded-lg shadow-md p-6 max-w-6xl mx-auto">
        <h2 className="text-2xl font-bold text-[#2B2B80] mb-6">Add New Bike</h2>
